...
public class ReturnBook extends JFrame implements ActionListener{
    ...
    public void delete() {
        try {
            conn con = new conn();
            String sql = "delete from bookService where service_id=?";
	...
	contentPane.add(lblName);
        ...
        JLabel lblCourse = new JLabel("services");
	...
	JLabel lblDateOfIssue = new JLabel("Date of booking");
	...
	JLabel lblDateOfReturn = new JLabel("Date of withdraw");
	...
	panel.setBorder(new TitledBorder(new LineBorder(new Color(255, 69, 0), 2, true), "Withdraw-Panel",
    ...
    public void actionPerformed(ActionEvent ae){
        try{
            conn con = new conn();
            if(ae.getSource() == b1){
                String sql = "select * from bookService where user_id = ? and service_id =?";
		PreparedStatement st = con.c.prepareStatement(sql);
		...
		ResultSet rs = st.executeQuery();
                ...
                while (rs.next()) {
                    ...
                    textField_4.setText(rs.getString("services"));
                    textField_5.setText(rs.getString("price"));
                    textField_6.setText(rs.getString("dateOfBooking"));
		}
		st.close();
		rs.close();
            ...
            if(ae.getSource() == b2){
                String sql = "insert into returnBook(service_id, user_id, bname, sname, services, price, dateOfBooking, dateOfWithdraw) values(?, ?, ?, ?, ?, ?, ?, ?)";
		PreparedStatement st = con.c.prepareStatement(sql);
		st.setString(1, textField.getText());
